...
; (def KnownScript
;   (schema/enum :onboard :onboard-user :onboard-user-authenticated :story))
...
; (defn ->trigger [org story share-request origin-url user]
;   (let [slack-org-id (-> share-request :channel :slack-org-id)
;         channel (-> ctx :channel :channel-id)
;         everyone? (or (= channel "__everyone__") (nil? channel))]
;     {:bot         (:slack-bot ctx)
;      :script      {:id :story, :params (-> (select-keys update [:slug :created-at])
